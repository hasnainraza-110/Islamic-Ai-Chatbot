...
from aichatbot.models import Hadith, Book

class Command(BaseCommand):
    help = 'Import Hadith from a text file into the database and associate them with a book'

    def handle(self, *args, **kwargs):
        file_path = 'cleaned_hadithmatchedmatched_hadiths_outputextracted_hadithمعاملات.txt'
        ...
        try:
            ...
            for hadith in hadith_list:
                if hadith.strip():  # Avoid inserting empty strings
                    Hadith.objects.create(text=hadith.strip(), book=book)
